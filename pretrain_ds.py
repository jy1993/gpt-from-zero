...
parser.add_argument('--zh_cache_path', type=str, default='cached/zh')
parser.add_argument('--en_cache_path', type=str, default='cached/en')
	...
	for _ in trange(args.epochs):
		for i, batch in enumerate(tqdm(train_loader)):
			...
			if global_step % args.save_steps == 0:
				...
				torch.save(sd, args.output_dir + '/%s_steps_%s' % (prefix, global_step))
...
def main():
	...
	if args.task == 'pretrain':
		...
		train_dataset = interleave_datasets([dataset_zh_dict['train'], dataset_en_dict['train'], dataset_code_dict['train']])
		val_dataset = interleave_datasets([dataset_zh_dict['validation'], dataset_en_dict['validation'], dataset_code_dict['validation']])
		dataset_dict = {'train': train_dataset, 'validation': val_dataset}
		print(dataset_dict)
	elif args.task == 'sft':
		dataset_dict = load_from_disk(args.alpaca_cache_path)
	print('done loading data')
	if args.global_rank <= 0 and args.task == 'sft':
