...
class SelfAttn(nn.Module):
	...
	def forward(self, query, key, value, mask=None, position_ids=None, layer_past=None, use_cache=False):
		'''
		Parameters:
...
class ExpertDecoderLayer(nn.Module):
	"""docstring for ExpertDecoderLayer"""
	def __init__(self, num_heads, num_hid_dim, hidden_size, dropout, mid_size, num_of_experts, experts_per_token):
		super(ExpertDecoderLayer, self).__init__()
		self.attn = SelfAttn(num_heads, num_hid_dim, hidden_size, dropout)
		self.router = nn.Linear(hidden_size, num_of_experts)
		self.experts = nn.ModuleList([FeedForward(hidden_size, mid_size, dropout) for _ in range(num_of_experts)])
		self.input_norm = RMSNorm(hidden_size)
		self.post_attention_norm = RMSNorm(hidden_size)
	...
	def forward(self, hidden_states, attention_mask=None, position_ids=None, layer_past=None, use_cache=False):
		...
		residual = hidden_states
		hidden_states = self.post_attention_norm(hidden_states)
		hidden_states = self.expert_choice(hidden_states)
		hidden_states = hidden_states + residual
		return hidden_states, layer_past

	def expert_choice(self, hidden_states):
		# bsz, seq_len, num_of_experts
		probs = F.softmax(self.router(hidden_states), dim=-1)
		sorted_probs, sorted_indices = probs.sort(dim=1, descending=True)
		expert_capacity = hidden_states.shape[1] * self.experts_per_token // self.num_of_experts
		# bsz, expert_capacity, num_of_experts
		top_tokens_indices = sorted_indices[:, :expert_capacity]
		top_tokens_probs = sorted_probs[:, :expert_capacity]
		
		# print(probs[0])
		# print(sorted_probs[0])
		# print(sorted_indices[0])
		# print(expert_capacity)
		# print(top_tokens_indices[0])
...
class GPT(nn.Module):
	def __init__(self, config, generation_config=None):
		...
		self.layers = nn.ModuleList([ExpertDecoderLayer(config['num_attention_heads'], config['num_hid_dim'], config['hidden_size'], config['dropout'], config['mid_size'], config['num_of_experts'], config['experts_per_token']) if idx % 2 == 1 else DecoderLayer(config['num_attention_heads'], config['num_hid_dim'], config['hidden_size'], config['dropout'], config['mid_size']) for idx in range(config['num_layers'])])
	...
	def generate(self, input_ids, position_ids=None, attention_mask=None):
		...
		while True:
			...
			for idx, layer in enumerate(self.layers):
				hidden_states, layer_past = layer(hidden_states, attention_mask=attention_mask, position_ids=model_inputs['position_ids'], layer_past=past_key_values[idx], use_cache=self.generation_config['use_cache'])
				if self.generation_config['use_cache']:
